...
import { readMessList } from '../repository';
import Add from './Add';
...
function Index() {
  ...
  const [currentMess, setCurrentMess] = useState<MessInterface>(initialvalue)

  const [showFilterOption, setShowFilterOption] = useState(false);
  const [siteFilter, setSiteFilter] = useState("all")
  const [centreFilter, setCentreFilter] = useState("")
  const [searchQuery, setSearchQuery] = useState("")
  const [hallList, setHallList] = useState<SelectedData[]>([])
  ...
  const handleEditButton = (mess: MessInterface) => {
    setCurrentMess(mess);
    setShowModal("edit")
  }
  const handlePasswordButton = (mess: MessInterface) => {
    setCurrentMess(mess);
